...
interface UploadRentAgregadasModalProps {
  onClose: () => void;
}
...
export default function UploadRentAgregadasModal({ onClose }: UploadRentAgregadasModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('fecha_calculo', fechaCalculo);
      formData.append('modo', modo);

      const response = await fetch('/api/rentabilidades-agregadas', {
        method: 'POST',
        body: formData
      });
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '24px' }}>
          <h2 style={{ 
            ...
            📊 Cargar Rentabilidades Agregadas
          </h2>
          <div style={{ fontSize: '13px', color: '#666' }}>
            Carga masiva de rentabilidades por período (7d, 30d, 1y, etc.)
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#0c4a6e', fontFamily: 'monospace' }}>
            fo_run | fm_serie | rent_7d | rent_30d | rent_365d | rent_ytd | ...
          </div>
          <div style={{ fontSize: '11px', color: '#0369a1', marginTop: '8px' }}>
            Columnas opcionales: rent_90d, rent_180d, rent_3y, rent_5y, volatilidad_30d, sharpe_365d, patrimonio_mm
          </div>
        </div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <label style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: '#666' }}>
                  Borra datos de esta fecha e inserta nuevos
                </div>
              </div>
            </label>
            ...
            <label style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: '#666' }}>
                  Mantiene datos existentes y agrega nuevos
                </div>
              </div>
            </label>
          </div>
        ...
        {result && (
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#047857' }}>
              • {result.insertados} registros insertados
              {result.errores > 0 && ` • ${result.errores} errores`}
            </div>
            <div style={{ fontSize: '11px', color: '#059669', marginTop: '4px' }}>
              Fecha: {result.fecha_calculo} | Modo: {result.modo}
            </div>
