...
const OBS_COLORS: Record<string, { border: string; bg: string; text: string }> = {
  alta: { border: "#EF5B5B", bg: "#fef2f2", text: "#991b1b" },
...
function buildDonutSVG(
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
    <circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="#F7F6F2" stroke-width="${strokeWidth}"/>
    ${circles}
    <text x="${size / 2}" y="${size / 2 + 4}" text-anchor="middle" font-size="12" fill="#6E7787" font-family="${FONT}">${escapeHtml(sublabel)}</text>
  </svg>`;
}
...
function buildAllocationSection(allocation: Record<string, { actual: number; target: number; delta: number }>): string {
    ...
    .map((role) => {
      ...
      <div style="margin-bottom:14px;">
        <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:4px;">
          <span style="color:#0B2140; font-weight:500; font-family:${FONT};">${ROLE_LABELS[role]}</span>
        ...
        <div style="display:flex; align-items:center; gap:8px; margin-bottom:2px;">
          <span style="font-size:10px; color:#a9a49c; width:40px; font-family:${FONT};">Actual</span>
          <div style="flex:1; height:14px; background:#F7F6F2; border-radius:7px; overflow:hidden;">
            <div style="width:${Math.min((a.actual / maxPct) * 100, 100).toFixed(0)}%; height:100%; background:${ROLE_COLORS[role]}; border-radius:7px;"></div>
          </div>
          <span style="font-size:11px; font-family:monospace; width:40px; text-align:right; font-weight:600; color:#0B2140;">${a.actual.toFixed(1)}%</span>
        </div>
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="font-size:10px; color:#a9a49c; width:40px; font-family:${FONT};">Modelo</span>
          <div style="flex:1; height:14px; background:#F7F6F2; border-radius:7px; overflow:hidden;">
          ...
          <span style="font-size:11px; font-family:monospace; color:#a9a49c; width:40px; text-align:right;">${a.target.toFixed(1)}%</span>
        </div>
      </div>`;
    })
    .join("");
  ...
  return `
    <div style="padding:24px 32px; border-bottom:1px solid #E7E4DD;">
      <div style="font-size:14px; font-weight:600; color:#0B2140; margin-bottom:16px; font-family:${FONT};">Asset Allocation vs Modelo</div>
      ${bars}
      <div style="display:flex; justify-content:center; gap:40px; margin-top:20px;">
        <div style="text-align:center;">
          <div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#a9a49c; margin-bottom:8px; font-family:${FONT};">Tu Cartera</div>
          ${actualDonut}
        </div>
        <div style="text-align:center;">
          <div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#a9a49c; margin-bottom:8px; font-family:${FONT};">Modelo</div>
          ${targetDonut}
        </div>
      </div>
    </div>`;
...
function buildHoldingsSection(ib: RadiografiaEmailData["instrumentBreakdown"]): string {
    ...
    .map(
      (r) => `
        <tr style="border-bottom:1px solid #F7F6F2;">
          <td style="padding:8px 0; font-weight:600; color:#0B2140; font-family:${FONT};">${r.ticker ? `<span style="font-family:monospace; background:#F7F6F2; padding:2px 6px; border-radius:4px; margin-right:6px; font-size:11px;">${escapeHtml(r.ticker)}</span>` : ""}${escapeHtml(r.name)}</td>
          <td style="padding:8px 0; color:#6E7787; font-family:${FONT}; font-size:12px;">${r.tipo}</td>
          <td style="padding:8px 0; text-align:right; font-family:monospace; font-weight:600; font-size:12px;">${r.weightPct.toFixed(1)}%</td>
          <td style="padding:8px 0; text-align:right; font-family:monospace; font-size:12px; color:#6E7787;">${r.valor}</td>
        </tr>`,
    )
    .join("");
  ...
  return `
    <div style="padding:24px 32px; border-bottom:1px solid #E7E4DD;">
      <div style="font-size:14px; font-weight:600; color:#0B2140; margin-bottom:12px; font-family:${FONT};">Principales Posiciones</div>
      <table style="width:100%; border-collapse:collapse; font-size:12px;">
        <tr style="border-bottom:1px solid #F7F6F2;">
          <th style="text-align:left; padding:6px 0; color:#a9a49c; font-weight:500; font-family:${FONT};">Instrumento</th>
          <th style="text-align:left; padding:6px 0; color:#a9a49c; font-weight:500; font-family:${FONT};">Tipo</th>
          <th style="text-align:right; padding:6px 0; color:#a9a49c; font-weight:500; font-family:${FONT};">Peso</th>
          <th style="text-align:right; padding:6px 0; color:#a9a49c; font-weight:500; font-family:${FONT};">Valor</th>
        </tr>
        ${rowsHtml}
      </table>
    </div>`;
...
function buildObservationsSection(observations: RadiografiaEmailData["observations"]): string {
  ...
  return `
    <div style="padding:24px 32px; border-bottom:1px solid #E7E4DD;">
      <div style="font-size:14px; font-weight:600; color:#0B2140; margin-bottom:12px; font-family:${FONT};">Observaciones</div>
      ${items}
    </div>`;
}
...
function buildNarrativeSection(narrative: string): string {
  const paragraphs = narrative
    .split("\n\n")
    .filter((p) => p.trim())
    .map((p) => `<div style="font-size:13px; color:#6E7787; line-height:1.7; margin-bottom:12px; font-family:${FONT};">${escapeHtml(p)}</div>`)
    .join("");

  return `
    <div style="padding:24px 32px; border-bottom:1px solid #E7E4DD;">
      <div style="font-size:14px; font-weight:600; color:#0B2140; margin-bottom:12px; font-family:${FONT};">Diagnostico</div>
      ${paragraphs}
    </div>`;
}
...
export function buildRadiografiaHTML(data: RadiografiaEmailData): string {
  ...
  const header = `
    <div style="background:#0B2140; color:white; padding:24px 32px;">
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
          <div style="font-size:11px; text-transform:uppercase; letter-spacing:1.5px; color:#a9a49c; margin-bottom:4px; font-family:${FONT};">Global</div>
        ...
        <div style="text-align:right;">
          ...
          <div style="font-size:12px; color:#a9a49c; font-family:${FONT};">${escapeHtml(data.reportDate)}</div>
        </div>
      </div>
      <div style="margin-top:12px; display:flex; gap:12px;">
        <div style="background:#6E7787; padding:6px 12px; border-radius:6px; font-size:12px; font-family:${FONT};">
          <span style="color:#a9a49c;">Perfil:</span> ${escapeHtml(profileLabel)}
        </div>
        <div style="background:#6E7787; padding:6px 12px; border-radius:6px; font-size:12px; font-family:${FONT};">
          <span style="color:#a9a49c;">Valor:</span> ${formatCLP(data.totalValueCLP)}
        </div>
        <div style="background:#6E7787; padding:6px 12px; border-radius:6px; font-size:12px; font-family:${FONT};">
          <span style="color:#a9a49c;">Modelo:</span> ${escapeHtml(modelLabel)}
        </div>
      </div>
    </div>`;
  ...
  const footer = `
    <div style="padding:20px 32px; background:#F7F6F2;">
      <div style="font-size:11px; color:#a9a49c; text-align:center; font-family:${FONT};">
        Global &mdash; Este reporte es informativo y no constituye recomendacion de inversion.
        <br/>Para ver la radiografia completa, <a href="${escapeHtml(data.platformUrl)}" style="color:#5AA0E6; text-decoration:underline;">ingresa a la plataforma</a>.
      </div>
    </div>`;
