...
def cargar_fondos_base(df_rent, df_cost):
    ...
    # Combinar datos únicos - INCLUIR nombre_fondo y nombre_agf de AMBOS archivos
    ...
    fondos_cost = df_cost[['fo_run', 'fm_serie', 'nombre_fondo', 'nombre_agf', 'moneda_funcional']].copy()
    
    # Merge con sufijos para identificar origen
    fondos = fondos_rent.merge(
        fondos_cost, 
        on=['fo_run', 'fm_serie'], 
        how='outer',
        suffixes=('_rent', '_cost')
    )
    
    # Combinar nombre_fondo: priorizar rentabilidades, sino costos
    fondos['nombre_fondo'] = fondos['nombre_fondo_rent'].fillna(fondos['nombre_fondo_cost'])
    fondos['nombre_agf'] = fondos['nombre_agf_rent'].fillna(fondos['nombre_agf_cost'])
    
    # Eliminar columnas duplicadas
    fondos = fondos.drop(columns=['nombre_fondo_rent', 'nombre_fondo_cost', 
                                   'nombre_agf_rent', 'nombre_agf_cost'])
    ...
    # IMPORTANTE: Asegurar que TODOS los objetos tengan las MISMAS columnas
    # Definir columnas obligatorias
    columnas_requeridas = [
        'fo_run', 'fm_serie', 'nombre_fondo', 'nombre_agf',
        'familia_estudios', 'familia_visualizador', 'familia_rar',
        'clase_inversionista', 'serie_digital', 'moneda_funcional'
    ]
    
    # Convertir a dict con TODAS las columnas
    fondos_data = []
    for _, row in fondos.iterrows():
        ...
        for col in columnas_requeridas:
            ...
            if pd.isna(val) or val is None:
                ...
                fondo[col] = int(val) if val == int(val) else float(val)
            else:
                fondo[col] = str(val)
...
def cargar_rentabilidades(df_rent, fondos_map):
    ...
    # Columnas requeridas para rentabilidades
    columnas_rent = [
        'fondo_id', 'fo_run', 'fm_serie', 'fm_fecha',
        'rent_nominal_1a', 'rent_nominal_3a_ann', 'rent_nominal_5a_ann', 'rent_nominal_10a_ann',
        'rent_real_1a', 'rent_real_3a_ann', 'rent_real_5a_ann', 'rent_real_10a_ann',
        'ind_rar_pond'
    ]
    ...
    for _, row in df_rent.iterrows():
        ...
        rent = {
            ...
            'fm_fecha': fecha,
            'rent_nominal_1a': float(row['rent_nominal_1a']) if pd.notna(row['rent_nominal_1a']) else None,
            'rent_nominal_3a_ann': float(row['rent_nominal_3a_ann']) if pd.notna(row['rent_nominal_3a_ann']) else None,
            'rent_nominal_5a_ann': float(row['rent_nominal_5a_ann']) if pd.notna(row['rent_nominal_5a_ann']) else None,
            'rent_nominal_10a_ann': float(row['rent_nominal_10a_ann']) if pd.notna(row['rent_nominal_10a_ann']) else None,
            'rent_real_1a': float(row['rent_real_1a']) if pd.notna(row['rent_real_1a']) else None,
            'rent_real_3a_ann': float(row['rent_real_3a_ann']) if pd.notna(row['rent_real_3a_ann']) else None,
            'rent_real_5a_ann': float(row['rent_real_5a_ann']) if pd.notna(row['rent_real_5a_ann']) else None,
            'rent_real_10a_ann': float(row['rent_real_10a_ann']) if pd.notna(row['rent_real_10a_ann']) else None,
            'ind_rar_pond': float(row['ind_rar_pond']) if pd.notna(row['ind_rar_pond']) else None
        }
...
def cargar_costos(df_cost, fondos_map):
    ...
    for _, row in df_cost.iterrows():
        ...
        cost = {
            ...
            'fo_run': int(row['fo_run']),
            'fm_serie': str(row['fm_serie']),
            'tac_sintetica': float(row['tac_sintetica']) if pd.notna(row['tac_sintetica']) else None,
            'pat_total': float(row['pat_total']) if pd.notna(row['pat_total']) else None,
            'fm_fecha_num': int(row['fm_fecha_num']) if pd.notna(row['fm_fecha_num']) else None
        }
